...
// Logs a warning when no notifier is active.
func NewNotifiers(tg *TelegramNotifier, dc *DiscordNotifier) Notifiers {
	...
	if len(ns) == 0 {
		log.Println("submitter: WARNING — no notifier configured (set TELEGRAM_BOT_TOKEN/TELEGRAM_CHAT_ID or DISCORD_WEBHOOK_URL)")
	}
	return ns
}
